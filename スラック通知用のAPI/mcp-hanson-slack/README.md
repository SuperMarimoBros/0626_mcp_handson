...
```bash
mcp-hanson-slack$ sam build --use-container
```
...
```bash
mcp-hanson-slack$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
mcp-hanson-slack$ sam local start-api
mcp-hanson-slack$ curl http://localhost:3000/
```
...
```bash
mcp-hanson-slack$ sam logs -n HelloWorldFunction --stack-name mcp-hanson-slack --tail
```
...
```bash
mcp-hanson-slack$ pip install -r tests/requirements.txt --user
# unit test
mcp-hanson-slack$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
mcp-hanson-slack$ AWS_SAM_STACK_NAME="mcp-hanson-slack" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "mcp-hanson-slack"
```
